---
name: chrisai-coding-ts-logic-review
description: Use this skill when reviewing existing TypeScript code and tests for complex branching, hard-to-follow decision logic, missing branch coverage, duplicated rules, mutation risk, or refactor opportunities before deciding whether to apply changes.
---
...
Use `chrisai-coding-ts` after the user approves code edits so the resulting
TypeScript follows implementation style, comments, imports, exports, and typing
rules.

Use `chrisai-coding-ts-tests` after the user approves test edits so added or
...
Read `references/review-signals.md` when the review needs concrete guidance on:
