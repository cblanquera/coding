#!/usr/bin/env python3
"""Create an HTML strip for favicon-size SVG/PNG review using valid favicon sizes."""
...
def build_card(path: Path, sizes: list[int]) -> str:
    suffix = path.suffix.lower()
    tiles: list[str] = []
    if suffix == ".svg":
        source = path.read_text(encoding="utf-8")
        tiles = [render_svg_tile(source, size) for size in sizes]
    elif suffix == ".png":
        data_uri = png_to_data_uri(path)
        tiles = [render_png_tile(data_uri, size) for size in sizes]
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        "--subtitle",
        default="Review valid favicon sizes only before ICO packaging or final icon export.",
    )
    ...
    for value in args.inputs:
        cards.append(build_card(Path(value), sizes))
