---
name: github-pages-qa-screenshot
description: Run QA and capture screenshots for a generated GitHub Pages site. Use this skill when Codex needs to start a localhost preview, validate responsive and visual behavior in the browser, diagnose preview blockers, and capture screenshots of the generated output.
---

# GitHub Pages QA Screenshot

Use this skill when the main problem is validating or capturing the generated
...
screenshot work. If the repo already exposes enough build and preview contract
...
- if Node 22+ cannot be resolved, report that as a blocker because preview
  startup and in-app browser QA may be invalid or unavailable without the
  correct runtime
...
- `.playwright/artifacts/` for optional screenshots or QA outputs when the
  workflow needs a durable local artifact path
...
- the user must actively invoke `@browser` in the request when Codex needs to
  use the in-app browser; do not assume in-app browser access is available
  otherwise
- use the Codex in-app browser for visual QA and screenshots, not an external
  browser
- if the in-app browser cannot be used, report that clearly instead of
  silently switching to Safari, Chrome, or another external browser

Before relying on the wrapper:

- ensure the target project's `serve.mjs` or equivalent preview entrypoint
  honors both `HOST` and `PORT`
...
- include whether the blocker came from unresolved Node 22+, failed `yarn`
  commands, preview startup failure, or inability to use the Codex in-app
  browser
...
- require the user to invoke `@browser` before attempting in-app browser QA or
  screenshots
- perform visual checks in the Codex in-app browser
...
- capture screenshots only after confirming the rendered page matches the
  generated output you intend to show
- if in-app browser QA could not be completed, say exactly why and whether the
  root cause was runtime resolution, preview startup, browser access, or a repo
  issue
...
- choose pages that demonstrate the relevant layout or bug clearly
- capture the homepage when visual direction changed materially
- capture at least one representative article page when shared layout changed
- note the viewport used for each capture
- capture screenshots from the Codex in-app browser, not an external browser
- if a screenshot is blocked by preview failure, report the exact blocker
  instead of pretending the visual QA completed
...
3. browser or screenshot coverage
